...
private boolean isOpen = false;
    ...
    private ConsumerMain(String[] args) {
        ...
        ServiceRequestForm tempSRF = compileSRF("TemperatureSensor");
        ...
        //The tempProvider gets consumed (the temperature is being checked) every 250milliseconds and is sending the data to the controlServo-method
        try {
            while (true) {
                double temperature = consumeService(tempUrl);
                controlServo(servoUrl, temperature);
                Thread.sleep(250);
            }
        }
    ...
    private ServiceRequestForm compileSRF(String service_name) {
        ...
        */
        ...
        if(service_name.equals("TemperatureSensor")) {
          metadata.put("unit", "celsius");
        }
        else if(service_name.equals("Servo")){
    ...
    private void controlServo(String providerUrl, double temp){
        
        if(temp >= 25 && isOpen){
            Response getResponse = Utility.sendRequest(providerUrl, "POST", 200);
            isOpen = false;
        }
        else if(temp < 25 && !isOpen){
            Response getResponse = Utility.sendRequest(providerUrl, "POST", 100);
            isOpen = true;
        }
    }
